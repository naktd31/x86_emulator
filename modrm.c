...
{
  if(modrm->mod ==3){
    set_registers32(emu, modrm->rm, value);
  }else{
...
uint32_t calc_memory_address(Emulator* emu, ModRM* modrm)
{
  if(modrm->mod == 0){
    if(modrm->rm == 4){
      ...
      return get_registers32(emu, modrm->rm);
    }
  }else if(modrm->mod == 1){
    if(modrm->rm == 4){
      printf("not implemented ModRM mod =0, rm = 4\n");
      ...
      return get_registers32(emu, modrm->rm) + modrm->disp8;
    }
  }else if(modrm->mod == 2){
    if(modrm->rm == 4){
      printf("not implemented ModRM mod =0, rm = 4\n");
      ...
      return get_registers32(emu, modrm->rm) + modrm->disp32;
    }
...
{
  set_registers32(emu, modrm->reg_index, value);
}

uint32_t get_r32(Emulator* emu, ModRM* modrm)
{
  return get_registers32(emu, modrm->reg_index);
}
